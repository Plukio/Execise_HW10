...
class ApiProvider {
    ...
    fun findImage(id: String, cb: UnsplashResult) {
        retrofit.findImage(id).enqueue(object : Callback<ImageX> {
            override fun onResponse(call: Call<ImageX>, response: Response<ImageX>) {
                if (response.isSuccessful && response.body() != null) {
                    cb.onFindImageSuccess(response.body()!!)
                } else {
                    cb.onDataFetchedFailed()
                }
            }

            override fun onFailure(call: Call<ImageX>, t: Throwable) {
                cb.onDataFetchedFailed()
            }
        })
    }
